  ...
  request(url, function(err, response, html){
    ...
    for( var i=0; i < anchors.length; i++ ){
      ...
      if( (link.hostname === currentUrl.hostname || link.hostname === null) &&
          ...
          link.query === null && 
          link.hash === null &&
          // sidebar classes should be removed
          !parentHasClass(anchors[i], 
            ['infobox', 'hatnote', 'thumb', 'floatright', 'navbox']) &&
          !parentIs(anchors[i], 'i') &&
          !inParentheses( anchors[i]) &&
          currentUrl.href !== link.href ){

        // Have we been here before? Are we at the maxdepth?
        if( pathToEnlightment.indexOf(urlParser.format(link)) < 0 &&
          pathToEnlightment.length < MAX_DEPTH ){
          pathToEnlightment.push(url);
...
//Recursively travel up the tree looking for parent tags that contain 'classes'
function parentHasClass(el, classes){
  if( el.attribs && el.attribs.class ) {
    for( var i in classes ){
      if(el.attribs.class.search(classes[i]) >= 0 )
        return true;
    } 
  }
  if( !!el.parentNode )
    return parentHasClass(el.parentNode, classes);
